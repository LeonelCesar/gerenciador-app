...
// --- CLIENT GENERATOR ---
const names = ["Adão Domingos Costa", "Henriqueta César", "Eloa César", "Patrick Neves Pio", "Rafael Neves Pio", "Esteveny Sofecia", "Alberto Costa César", "Lanira Neves", "Henriques Dionisio", "António Liquini"];
...
const generateClients = (count: number): TransactionItem[] => {
  return Array.from({ length: count }, () => ({
    ...
    avatar: `https://i.pravatar.cc/40?img=${Math.floor(1 + Math.random() * 70)}`,
 ...
 const filteredData = useMemo(() => {
  ...
  const TableRow = ({ item }: { item: TransactionItem }) => (
    <tr className="text-sm hover:bg-stone-200/40 transition-colors">
      ...
      <td className="p-2 text-stone-600">{new Date(item.date).toLocaleDateString("en-US")}</td>
  ...
  return (
    <div className="col-span-12 p-4 rounded border border-stone-300 bg-white">
      ...
      <div className="flex gap-4 mb-4 text-sm">
        ...
        <select
          value={dateFilter}
          onChange={(e) => setDateFilter(e.target.value as "all" | "new" | "old")}
          className="border rounded p-1 text-sm"
        >
